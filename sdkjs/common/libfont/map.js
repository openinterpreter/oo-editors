...
(function(window, undefined){
	...
	function CApplicationFonts()
	{
		...
		this.GetFontFileWeb = function(name, lStyle)
		{
			...
			// NOTE(victor): Without this guard, documents referencing fonts not installed
			// locally (e.g. Calibri on macOS) trigger GetFontIndex's O(n) penalty-based
			// matching per missing font. With 50+ missing fonts this adds seconds to load.
			// Instead, return the default font (Arial) immediately.
			if (undefined === AscFonts.g_map_font_index[name])
			{
				var _fallbackFont = this.g_fontSelections.List[this.DefaultIndex];
				this.FontPickerMap[name] = _fallbackFont;
				if (window["onLogPickFont"])
					window["onLogPickFont"]("FontPicker: " + name + " => " + _fallbackFont.m_wsFontName + " (fallback)");
		...
		this.GetFontInfoWithoutEmbed = function(name, lStyle, objDst)
		{
			var _font = this.GetFontFileWeb(name, lStyle);
			var font_name_index = AscFonts.g_map_font_index[_font.m_wsFontName];
			// NOTE(victor): GetFontFileWeb may resolve to a font name that exists in the
			// selection list but not in g_map_font_index (e.g. when AllFonts.js and the
			// font selection data diverge). Without this fallback, g_font_infos[undefined]
			// propagates and crashes the editor.
			if (undefined === font_name_index)
				font_name_index = AscFonts.g_map_font_index["Times New Roman"];
